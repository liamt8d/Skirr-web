    ...
    try {
        ...
        if (!resendApiKey) {
            return NextResponse.json(
                { success: false, error: 'Falta RESEND_API_KEY en variables de entorno.' },
                { status: 500 }
            );
        }
        ...
        const discordMessage = {
            embeds: [{
                ...
                fields: [
                    ...
                    { name: 'Servicio', value: data.servicio || 'N/A', inline: true },
                    { name: 'Presupuesto', value: data.presupuesto || 'N/A', inline: true },
                    { name: 'Plazo', value: data.plazo || 'N/A', inline: true },
            ...
            await resend.emails.send({
                ...
                html: `
                    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background-color: #050508; color: #ffffff; padding: 30px; border-radius: 12px; border: 1px solid #2a2a35;">
                        ...
                        <p style="color: #d1d5db; line-height: 1.6;">Nuestro equipo esta revisando los detalles de tu comunidad y nos pondremos en contacto contigo muy pronto a traves de tu metodo preferido (${data.metodoContacto}).</p>
                        <br/>
                        ...
                        <p style="font-size: 12px; color: #6b7280; text-align: center;">Este es un mensaje automatico, por favor no respondas a este correo.</p>
                    </div>
                `
            });
        }

        return NextResponse.json({ success: true, message: 'Mensajes enviados con exito.' });
    } catch (error) {
